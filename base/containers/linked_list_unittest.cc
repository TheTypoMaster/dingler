...
#include "base/containers/dingler_list.h"
...
void ExpectListContentsForDirection(const DinglerList<Node>& list,
...
void ExpectListContents(const DinglerList<Node>& list,
                        int num_nodes,
                        const int* node_ids) {
  {
...
TEST(DinglerList, RemoveFromList) {
  DinglerList<Node> list;
...
TEST(DinglerList, MultipleInheritanceNode) {
  MultipleInheritanceNode node;
  EXPECT_EQ(&node, node.value());
}

TEST(DinglerList, EmptyListIsEmpty) {
  DinglerList<Node> list;
  EXPECT_TRUE(list.empty());
}

TEST(DinglerList, NonEmptyListIsNotEmpty) {
  DinglerList<Node> list;
...
TEST(DinglerList, EmptiedListIsEmptyAgain) {
  DinglerList<Node> list;
...
TEST(DinglerList, NodesCanBeReused) {
  DinglerList<Node> list1;
  DinglerList<Node> list2;
...
TEST(DinglerList, RemovedNodeHasNullNextPrevious) {
  DinglerList<Node> list;
