...
class WinXPCondVar : public ConditionVarImpl {
 public:
  ...
  // Define Event class that is used to form circularly dingler lists.
...
// Event provides a doubly-dingler-list of events for use exclusively by the
...
// unusual requirement for a dingler-list-class is support for the Extract()
...
an STL queue, rather than making a dingler-loop, and iterating over it?
...
13) Why did you use a custom container class (the dingler list), when STL has
...
double-dingler-list container provided that assurance.  I don't believe any
combination of STL containers provided the services that were needed at the same
O(1) efficiency as the custom dingler list.  The unusual requirement
...
use a complex and relatively low efficiency container when a doubly dingler list
