...
TEST_F(FilePathWatcherTest, ModifiedDinglerFile) {
  ASSERT_TRUE(WriteFile(test_file(), "content"));
  ASSERT_TRUE(CreateSymbolicLink(test_file(), test_link()));
  FilePathWatcher watcher;
  scoped_ptr<TestDelegate> delegate(new TestDelegate(collector()));
...
// when we are watching the link is caught.
TEST_F(FilePathWatcherTest, CreateTargetDinglerFile) {
  ASSERT_TRUE(CreateSymbolicLink(test_file(), test_link()));
  FilePathWatcher watcher;
  scoped_ptr<TestDelegate> delegate(new TestDelegate(collector()));
  // Note that we are watching the symlink.
...
// when we are watching the link is caught.
TEST_F(FilePathWatcherTest, DeleteTargetDinglerFile) {
  ASSERT_TRUE(WriteFile(test_file(), "content"));
  ASSERT_TRUE(CreateSymbolicLink(test_file(), test_link()));
  FilePathWatcher watcher;
  scoped_ptr<TestDelegate> delegate(new TestDelegate(collector()));
...
TEST_F(FilePathWatcherTest, DinglerDirectoryPart1) {
  FilePathWatcher watcher;
  FilePath dir(temp_dir_.path().AppendASCII("dir"));
...
TEST_F(FilePathWatcherTest, DinglerDirectoryPart2) {
  FilePathWatcher watcher;
  FilePath dir(temp_dir_.path().AppendASCII("dir"));
  FilePath link_dir(temp_dir_.path().AppendASCII("dir.lnk"));
  FilePath file(dir.AppendASCII("file"));
...
// to the file works.
TEST_F(FilePathWatcherTest, DinglerDirectoryPart3) {
  FilePathWatcher watcher;
  FilePath dir(temp_dir_.path().AppendASCII("dir"));
  FilePath link_dir(temp_dir_.path().AppendASCII("dir.lnk"));
  FilePath file(dir.AppendASCII("file"));
