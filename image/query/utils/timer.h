...
// library.
#ifndef IMAGE_OPENSOURCE_TIMER_TIMER_H_
#define IMAGE_OPENSOURCE_TIMER_TIMER_H_
...
class TimerBase {
 public:
  ...
  virtual double Get() const = 0;        // get the value in seconds
  virtual int64 GetInUsec() const = 0;   // get the value in microseconds
  virtual int64 GetInNanos() const = 0;   // get the value in cycles
...
class WallTimer : public TimerBase {
 public:
  ...
  virtual int64 GetInUsec() const override;
  virtual int64 GetInNanos() const override;
};

inline double WallTimer::Get() const {
 return 0;//return toSeconds(timer_.elapsed().wall);
}

inline int64 WallTimer::GetInUsec() const {
 return 0;//return toUSeconds(timer_.elapsed().wall);
}

inline int64 WallTimer::GetInNanos() const {
 return 0;//return timer_.elapsed().wall;
}
...
inline double UserTimer::Get() const {
 return 0;//return toSeconds(timer_.elapsed().user);
}

inline int64 UserTimer::GetInUsec() const {
 return 0;//return toUSeconds(timer_.elapsed().user);
}

inline int64 UserTimer::GetInNanos() const {
 return 0;//return timer_.elapsed().user;
}
...
inline double SystemTimer::Get() const {
 return 0;//return toSeconds(timer_.elapsed().system);
}

inline int64 SystemTimer::GetInUsec() const {
 return 0;//return toUSeconds(timer_.elapsed().system);
}

inline int64 SystemTimer::GetInNanos() const {
 return 0;//return timer_.elapsed().system;
}

#endif  // IMAGE_OPENSOURCE_TIMER_TIMER_H_
