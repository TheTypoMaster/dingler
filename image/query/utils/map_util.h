...
FindDinglerPtrOrNull(const Collection& collection,
                    const typename Collection::value_type::first_type& key) {
  typename Collection::const_iterator it = collection.find(key);
  if (it == collection.end()) {
    return 0;
...
template <class Collection>
typename Collection::value_type::second_type::element_type&
FindDinglerPtrOrDie(const Collection& collection,
                   const typename Collection::value_type::first_type& key) {
  typename Collection::const_iterator it = collection.find(key);
...
//
// Use LookupOrInsertNewDinglerPtr if the container owns the elements.
...
// cannot be done with FindDinglerPtr*() even though it compiles.
...
// A variant of LookupOrInsertNewDinglerPtr where the value is constructed using
...
// LookupOrInsertNewDinglerPtr, this function returns the shared_ptr instead of
...
ReverseCollection ReverseMap(const Collection& collection) {
  typename gtl::internal::RemoveConst<ReverseCollection>::type reverse;
