...
#ifndef DINGLER_BASE_CASTS_H_
#define DINGLER_BASE_CASTS_H_
...
inline To implicit_cast(typename container::identity_<To>::type to) {
  return to;
}
...
inline To implicit_cast(typename container::identity_<From>::type const &f) {
  return f;
}
...
inline Enum loose_enum_cast(int e_val) {
  if (!loose_enum_test<Enum>(e_val)) {
   container::internal::WarnEnumCastError(e_val);
  }
