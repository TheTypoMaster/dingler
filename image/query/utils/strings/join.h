...
//#include "supersonic/utils/std_namespace.h"
#include <string>
namespace strings {using std::string; }
#include <utility>
//#include "supersonic/utils/std_namespace.h"
#include <vector>
using std::vector;
...
#include "image/query/utils/strings/join_internal.h"
#include "image/query/utils/strings/numbers.h"
#include "image/query/utils/strings/strcat.h"    // For backward compatibility.
...
PairFormatter(FirstFormatter f1, base::StringPiece sep, SecondFormatter f2) {
...
inline internal::PairFormatterImpl<
  ...
  internal::AlphaNumFormatterImpl>
PairFormatter(base::StringPiece sep) {
...
std::string Join(Iterator start, Iterator end, base::StringPiece sep, Formatter fmt) {
  return internal::JoinAlgorithm(start, end, sep, fmt);
}

template <typename Range, typename Formatter>
std::string Join(const Range& range, base::StringPiece separator, Formatter fmt) {
...
string Join(Iterator start, Iterator end, base::StringPiece separator) {
...
string Join(const Range& range, base::StringPiece separator) {
...
//    ',' as the delimiter.
// ----------------------------------------------------------------------
void JoinCSVLine(const vector<std::string>& original_cols, std::string* output);
std::string JoinCSVLine(const vector<std::string>& original_cols);
void JoinCSVLineWithDelimiter(const vector<std::string>& original_cols,
                              char delimiter,
                              std::string* output);
