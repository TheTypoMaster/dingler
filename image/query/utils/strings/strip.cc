...
std::string StripPrefixString(base::StringPiece str, base::StringPiece prefix) {
  if (str.starts_with(prefix))
    str.remove_prefix(prefix.length());
  return str.as_string();
}

bool TryStripPrefixString(base::StringPiece str, base::StringPiece prefix,
                                 std::string* result) {
...
std::string StripSuffixString(base::StringPiece str, base::StringPiece suffix) {
  if (str.ends_with(suffix))
    str.remove_suffix(suffix.length());
  return str.as_string();
}

bool TryStripSuffixString(base::StringPiece str, base::StringPiece suffix,
                                 std::string* result) {
...
// ----------------------------------------------------------------------
void StripString(char* str, base::StringPiece remove, char replacewith) {
  for (; *str != '\0'; ++str) {
   if (remove.find(*str) != base::StringPiece::npos) {
      *str = replacewith;
    }
  }
}

void StripString(char* str, int len, base::StringPiece remove, char replacewith) {
  char* end = str + len;
  for (; str < end; ++str) {
   if (remove.find(*str) != base::StringPiece::npos) {
      *str = replacewith;
...
void StripString(std::string* s, base::StringPiece remove, char replacewith) {
  for (std::string::iterator it = s->begin(), end = s->end(); it != end; ++it) {
   if (remove.find(*it) != base::StringPiece::npos) {
      *it = replacewith;
    }
  }
}
...
void StripBrackets(char left, char right, std::string* s) {
  std::string::iterator opencurly = find(s->begin(), s->end(), left);
  while (opencurly != s->end()) {
    std::string::iterator closecurly = find(opencurly, s->end(), right);
    if (closecurly == s->end())
      return;
...
void StripMarkupTags(std::string* s) {
  std::string::iterator output = find(s->begin(), s->end(), '<');
...
int TrimStringLeft(std::string* s, base::StringPiece remove) {
  int i = 0;
...
int TrimStringRight(std::string* s, base::StringPiece remove) {
...
void StripTrailingWhitespace(std::string* const s) {
