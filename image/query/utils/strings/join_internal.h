...
#include "image/query/utils/strings/strcat.h"
...
struct AlphaNumFormatterImpl {
  void operator()(std::string* out, const AlphaNum& a) const {
...
class PairFormatterImpl {
 public:
  PairFormatterImpl(F1 f1, base::StringPiece sep, F2 f2)
      : f1_(f1), sep_(sep.as_string()), f2_(f2) {}
...
class DereferenceFormatterImpl {
 public:
  ...
  template <typename T>
  void operator()(std::string* out, T* t) const {
    f_(out, *t);
  }
...
template <> struct DefaultFormatter<base::StringPiece> {
  typedef NoFormatter Type;
};
...
std::string JoinAlgorithm(Iterator start, Iterator end, base::StringPiece s, Formatter f) {
  std::string result;
  base::StringPiece sep;
...
std::string JoinAlgorithm(Iterator start, Iterator end, base::StringPiece s, NoFormatter) {
  std::string result;
