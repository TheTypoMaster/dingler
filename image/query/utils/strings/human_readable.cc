...
#include "image/query/utils/strings/human_readable.h"
...
bool HumanReadableNumBytes::LessThan(const std::string &a, const std::string &b) {
...
bool HumanReadableNumBytes::ToInt64(const std::string &str, int64 *num_bytes) {
  char *end;
...
bool HumanReadableNumBytes::ToDouble(const std::string &str, double *num_bytes) {
  char *end;
  double d = strtod(str.c_str(), &end);
  // If this didn't consume the entire string, fail.
  if ((end - str.c_str()) + 1 < str.size())
...
std::string HumanReadableNumBytes::DoubleToString(double num_bytes) {
  ...
  if (i == arraysize(units)) {
    return base::StringPrintf("%s%g", neg_str, num_bytes);
  } else {
   return base::StringPrintf("%s%.2f%c", neg_str, scaled, units[i]);
  }
}

std::string HumanReadableNumBytes::ToString(int64 num_bytes) {
  ...
  if (num_bytes < GG_LONGLONG(1024)) {
    ...
    return base::StringPrintf("%s%" GG_LL_FORMAT "dB", neg_str, num_bytes);
  }
  ...
  return base::StringPrintf(((*unit == 'K')
                       ? "%s%.1f%c"
                       : "%s%.2f%c"), neg_str, num_bytes / 1024.0, *unit);
}

std::string HumanReadableNumBytes::ToStringWithoutRounding(int64 num_bytes) {
  if (num_bytes == kint64min) {
    // Special case for number with not representable nagation.
    return "-8E";
  }
  ...
  return base::StringPrintf("%s%lld%c", neg_str, num_units, units[unit_type]);
}

std::string HumanReadableInt::ToString(int64 value) {
 std::string s;
  if (value < 0) {
    s += "-";
    value = -value;
  ...
  if (value < GG_LONGLONG(1000)) {
    base::StringAppendF(&s, "%" GG_LL_FORMAT "d", value);
  } else if (value >= GG_LONGLONG(1000000000000000)) {
    // Number bigger than 1E15; use that notation.
    base::StringAppendF(&s, "%0.3G", static_cast<double>(value));
  } else {
    ...
    base::StringAppendF(&s, "%.2f%c", value / 1000.0, *unit);
  }
  return s;
}

std::string HumanReadableNum::ToString(int64 value) {
...
std::string HumanReadableNum::DoubleToString(double value) {
 std::string s;
  ...
  if (value < 1.0) {
    base::StringAppendF(&s, "%.3f", value);
  } else if (value < 10) {
    base::StringAppendF(&s, "%.2f", value);
  } else if (value < 1e2) {
    base::StringAppendF(&s, "%.1f", value);
  } else if (value < 1e3) {
    base::StringAppendF(&s, "%.0f", value);
  } else if (value >= 1e15) {
    // Number bigger than 1E15; use that notation.
    base::StringAppendF(&s, "%0.3G", value);
  } else {
    static const char units[] = "kMBT";
    const char *unit = units;
    while (value >= 1e6) {
...
bool HumanReadableNum::ToDouble(const std::string &str, double *value) {
  char *end;
  double d = strtod(str.c_str(), &end);
...
bool HumanReadableInt::ToInt64(const std::string &str, int64 *value) {
  char *end;
  double d = strtod(str.c_str(), &end);
...
std::string HumanReadableElapsedTime::ToShortString(double seconds) {
 std::string human_readable;
  ...
  if (microseconds < 999.5) {
    base::StringAppendF(&human_readable, "%0.3g us", microseconds);
  ...
  if (milliseconds < 999.5) {
    base::StringAppendF(&human_readable, "%0.3g ms", milliseconds);
    return human_readable;
  }
  if (seconds < 60.0) {
    base::StringAppendF(&human_readable, "%0.3g s", seconds);
    return human_readable;
  }
  seconds /= 60.0;
  if (seconds < 60.0) {
    base::StringAppendF(&human_readable, "%0.3g min", seconds);
    return human_readable;
  }
  seconds /= 60.0;
  if (seconds < 24.0) {
    base::StringAppendF(&human_readable, "%0.3g h", seconds);
    return human_readable;
  }
  seconds /= 24.0;
  if (seconds < 30.0) {
    base::StringAppendF(&human_readable, "%0.3g days", seconds);
    return human_readable;
  }
  if (seconds < 365.2425) {
    base::StringAppendF(&human_readable, "%0.3g months", seconds / 30.436875);
    return human_readable;
  }
  seconds /= 365.2425;
  base::StringAppendF(&human_readable, "%0.3g years", seconds);
  return human_readable;
}

bool HumanReadableElapsedTime::ToDouble(const std::string& str, double* value) {
  struct TimeUnits {
