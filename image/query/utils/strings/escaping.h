...
//
// This is a grab-bag file for std::string utilities involved in escaping and
// unescaping std::strings in various ways. Who knew there were so many?
//
// There are more escaping functions in:
//   webutil/html/tagutils.h (Escaping std::strings for HTML, PRE, JavaScript, etc.)
...
#include "image/query/utils/strings/charset.h"
...
//
//    Example: [some "std::string" to test] --> [some ""std::string"" to test]
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
bool CUnescape(base::StringPiece source, char* dest, int* dest_len,
               std::string* error);

bool CUnescape(base::StringPiece source, std::string* dest, std::string* error);

// A version with no error reporting.
inline bool CUnescape(base::StringPiece source, std::string* dest) {
...
// display-formatted std::string that can be interpreted as a null-terminated
// const char* and will not be cut short if it contains embedded null
// characters.
//
// ----------------------------------------------------------------------

bool CUnescapeForNullTerminatedString(base::StringPiece source,
                                      char* dest,
                                      int* dest_len,
                                      std::string* error);
...
// A version with no error reporting.
inline bool CUnescapeForNullTerminatedString(base::StringPiece source,
                                             std::string* dest) {
...
//    Escaped chars: \n, \r, \t, ", ', \, and !ascii_isprint().
// ----------------------------------------------------------------------
std::string CEscape(base::StringPiece src);
std::string CHexEscape(base::StringPiece src);
std::string Utf8SafeCEscape(base::StringPiece src);
std::string Utf8SafeCHexEscape(base::StringPiece src);
...
// SYNOPSIS:
//
//   #include "std::strings/escaping.h"
//
//   #include "std::strings/charset.h"
//   #include "std::strings/std::stringpiece.h"
//
//   namespace std::strings {
//     void BackslashEscape(StringPiece src,
//                          const std::strings::CharSet& delims,
//                          std::string* dest);
//     void BackslashUnescape(StringPiece src,
//                            const std::strings::CharSet& delims,
//                            std::string* dest);
//     std::stringpiece_ssize_type BackslashUnescapedFind(
//         StringPiece src,
//         const std::strings::CharSet& delims);
//   }
//
...
//
//   std::strings::CharSet& delims
//      A std::strings::CharSet of characters to be considered escaped or
//      unescaped by the operation.
//
//      NOTE: Please use the explicit std::strings::CharSet(StringPiece)
//      constructor, as in:
//
//        std::strings::BackslashEscape(src, std::strings::CharSet(":,&\\"), &dest);
//
//   std::string* dest
//      Destination std::string Appended to by std::strings::BackslashEscape and
//      std::strings::BackslashUnescape.
//
// DESCRIPTION:
//
//  void BackslashEscape(StringPiece src, const std::strings::CharSet& delims,
//                       std::string* dest);
//
//    Replace any instance of a member 'c' of the std::strings::CharSet with
//    { '\\', c }.
//    For example, when exporting maps to /varz, label values need to
//    have all dots escaped.  Appends the result to dest.
//
//  void BackslashUnescape(StringPiece src, const std::strings::CharSet& delims,
//                         std::string* dest);
//
...
//    std::stringpiece_ssize_type) if no unescaped members of 'delims' are found.
//
// NOTE:
//
//    These functions do not escape or unescape backslash '\' by default.
//
//    For all std::strings, std::strings::BackslashUnescape is the exact inverse of
//    the std::strings::BackslashEscape with the matching 'delims' argument.
//    That is, for any StringPiece 'str' and any std::strings::CharSet 'delims':
//
//        std::string RoundTrip(StringPiece src, const std::strings::CharSet& delims) {
//          std::string encoded = std::strings::BackslashEscape(src, delims);
//          return std::strings::BackslashUnescape(encoded, delims);
//        }
//
...
//
//    Example 1:
//      Join arbitrary std::string fields with ':'.
//      Any ':' occurring in any of the fields will be escaped.
//
//    vector<std::string> fields = ...;
//    const std::strings::CharSet kDelims = ":";
//    std::string encoded;
//    const char* sep = "";
//    for (int i = 0; i < fields.size(); ++i) {
//      const std::string& field = fields[i];
//      StrAppend(&encoded, sep);
//      std::strings::BackslashEscape(field, kDelims, &encoded);
//      sep = ":";
//    }
//
//    Example 2:
//      Find the field boundaries in such an encoded std::string.
//
//      StringPiece encoded_sp = ...;  // from example 1
//      const std::strings::CharSet kDelims = ":";
//      vector<StringPiece> fields;
//      while (!encoded_sp.empty()) {
//        std::stringpiece_ssize_type pos =
//            std::strings::BackslashUnescapedFind(encoded_sp, kDelims);
...
//      const std::strings::CharSet kDelims = ":";
//      vector<std::string> decoded_fields;
//      for (int i = 0; i < encoded_fields.size(); ++i) {
//         StringPiece enc = encoded_fields[i];
//         std::string f;
//         std::strings::BackslashUnescape(enc, kDelims, &f);
//         decoded_fields.push_back(f);
//      }
//
// ----------------------------------------------------------------------
void BackslashEscape(base::StringPiece src, const CharSet& delims, std::string* dest);
void BackslashUnescape(base::StringPiece src, const CharSet& delims, std::string* dest);
base::StringPiece::size_type BackslashUnescapedFind(base::StringPiece src,
                                              const CharSet& delims);

// Convenience overload of BackslashEscape that returns a std::string value.
inline std::string BackslashEscape(base::StringPiece src, const CharSet& delims) {
  std::string s;
  BackslashEscape(src, delims, &s);
  return s;
}

// Convenience overload of BackslashUnescape that returns a std::string value.
inline std::string BackslashUnescape(base::StringPiece src, const CharSet& delims) {
  std::string s;
...
//    The variations that store into a std::string clear the std::string first, and
//    return false (with dest empty) if src contains invalid chars; for
//    these versions src and dest must be different std::strings.
// ----------------------------------------------------------------------
...
//   to and from binary data. We convert to and from std::string representations
...
// EscapeFileName()
// UnescapeFileName()
//   Utility functions to (un)escape std::strings to make them suitable for use in
...
//   understand the escaped form when the original std::string is a file path.
//
...
//   CFS.
//
//   The versions that receive a std::string for the output will append to it.
// ----------------------------------------------------------------------
void EscapeFileName(base::StringPiece src, std::string* dst);
void UnescapeFileName(base::StringPiece src, std::string* dst);
inline std::string EscapeFileName(base::StringPiece src) {
  std::string r;
  EscapeFileName(src, &r);
  return r;
}
inline std::string UnescapeFileName(base::StringPiece src) {
  std::string r;
...
//  Description: Binary-to-Ascii hex conversion.  This converts
//   'num' bytes of binary to a 2*'num'-character hexadecimal representation
//    Return value: 2*'num' characters of ascii std::string
// ----------------------------------------------------------------------
std::string b2a_hex(const char* from, int num);
std::string b2a_hex(base::StringPiece b);
...
//   first in the std::string if byte_order_msb is set.
...
// ----------------------------------------------------------------------
// ShellEscape
//   Make a shell command argument from a std::string.
//   Returns a Bourne shell std::string literal such that, once the shell finishes
...
//         with other shells.
// ----------------------------------------------------------------------
std::string ShellEscape(base::StringPiece src);
...
// Reads at most bytes_to_read from binary_std::string and writes it to
// ascii_std::string in lower case hex.
...
// Converts the hex from ascii_std::string into binary data and
// writes the binary data into binary_std::string.
// Empty input successfully converts to empty output.
// Returns false and may modify output if it is
// unable to parse the hex std::string.
bool ByteStringFromAscii(const std::string& ascii_string, std::string* binary_string);

// Clean up a multi-line std::string to conform to Unix line endings.
// Reads from src and appends to dst, so usually dst should be empty.
// If there is no line ending at the end of a non-empty std::string, it can
...
//
//   @param src The multi-line std::string to convert
//   @param dst The converted std::string is appended to this std::string
