...
//#include "image/query/utils/port.h"  // for LANG_CXX11
#include "image/query/utils/template_util.h"
#include <type_traits>
...
struct NoFilter {
  bool operator()(base::StringPiece /* ignored */) {
    return true;
  }
};
...
class ConvertibleToStringPiece {
 ...
 public:
  ...
  ConvertibleToStringPiece(base::StringPiece s) : value(s) {}    // NOLINT
  ConvertibleToStringPiece(const std::string& s) : value(s) {}  // NOLINT
...
class SplitIterator
 ...
 public:
  ...
  SplitIterator(base::StringPiece text, Delimiter d)
      : text_(text), position_(0), delimiter_(d), predicate_(), is_end_(false) {
    ++(*this);
  }
  SplitIterator(base::StringPiece text, Delimiter d, Predicate p)
  ...
  SplitIterator(base::StringPiece text, const SplitIterator& other)
      : text_(text),
        position_(0),
  ...
  base::StringPiece operator*() { return curr_piece_; }
  base::StringPiece* operator->() { return &curr_piece_; }
  ...
  SplitIterator& operator++() {
    do {
      ...
      base::StringPiece found_delimiter = delimiter_.Find(text_, position_);
...
struct StringPieceTo {
  To operator()(base::StringPiece from) const {
    return To(from);
  }
};
...
struct StringPieceTo<std::string> {
 std::string operator()(base::StringPiece from) const {
...
struct HasMappedType {
  template <typename U> static container::small_ check(...);  // default: No
  template <typename U> static container::big_ check(typename U::mapped_type*);
  enum { value = sizeof(container::big_) == sizeof(check<T>(0)) };
};
...
struct HasValueType {
 template <typename U> static container::small_ check(...);  // default: No
 template <typename U> static container::big_ check(typename U::value_type*);
 enum { value = sizeof(container::big_) == sizeof(check<T>(0)) };
};
...
struct HasConstIterator {
 template <typename U> static container::small_ check(...);  // default: No
 template <typename U> static container::big_ check(typename U::const_iterator*);
 enum { value = sizeof(container::big_) == sizeof(check<T>(0)) };
};
...
struct IsInitializerList {
 static container::small_ check(...);  // default: No
#ifdef LANG_CXX11
  template <typename U> static base::big_ check(std::initializer_list<U>*);
#endif  // LANG_CXX11
  enum { value = sizeof(container::big_) == sizeof(check(static_cast<T*>(0))) };
};
...
class Splitter {
 ...
 private:
  ...
  template <typename A>
  struct ContainerConverter<std::vector<std::string, A>, std::string, false> {
   std::vector<std::string, A> operator()(const Splitter& splitter) const {
     std::vector<base::StringPiece> vsp(splitter.begin(), splitter.end());
     std::vector<std::string, A> v(vsp.size());
