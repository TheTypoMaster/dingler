...
namespace image {
 ...
 /* static */
 string File::JoinPath(const std::string& dirname, const string& basename) {
 ...
 namespace {
...

  {
   ...
   struct stat tmp;
   base::FilePath path;
   path.AppendASCII(create_file_name_);
   ...
   else if (base::DirectoryExists(path)) {//(S_ISDIR(tmp.st_mode)) {
  ...
  if (must_use_fopen) {
   ...
   int fd;
#ifndef WIN32
   fd = open(file_name_.c_str(), mode_flags, permissions);
#else
   _sopen_s(&fd, file_name_.c_str(), mode_flags, _SH_DENYNO, permissions);
#endif
   if (fd >= 0) {
    internal_file_ = _fdopen(fd, file_mode_.c_str());
    if (internal_file_ == NULL) {
