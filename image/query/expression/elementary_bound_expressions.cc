...
#include "image/query/expression/elementary_bound_expressions.h"
...
#include "image/query/expression/terminal_expressions.h"
#include "image/query/expression/operators.pb.h"
#include "image/query/expression/abstract_bound_expressions.h"
#include "image/query/expression/bound_expression_factory.h"
...
#include "image/query/expression/vector_primitives.h"
#include "image/query/utils/strings/join.h"
#include "image/query/utils/fixedarray.h"
#include "image/query/utils/failureor.h"
...
FailureOrOwned<BoundExpression> BoundIfNull(BoundExpression* expression_ptr,
  ...
  if (!cast_expression->result_schema().attribute(0).is_nullable()) {
    return common::Success(cast_expression.release());
  }
...
FailureOrOwned<BoundExpression> BoundCase(BoundExpressionList* bound_arguments,
  ...
  if (args_ptr->size() < 2) {
    THROW(new Exception(
        container::ERROR_INVALID_ARGUMENT_VALUE,
