...
#include "image/query/expression/elementary_expressions.h"
...
#include "image/query/expression/abstract_expressions.h"
...
ChangeTypeExpression<OPERATOR_CAST_QUIET>::CreateBoundUnaryExpression(
  ...
  if (from_type == to_type_) return common::Success(child);
...
class CaseExpression : public Expression {
 public:
  ...
  virtual string ToString(bool verbose) const {
    return base::StringPrintf("CASE(%s)", arguments_->ToString(verbose).c_str());
  }
