...
#include "image/query/utils/mathlimits.h"
...
class BoundProjectionExpression : public BoundExpression {
 ...
 private:
  ...
  gtl::PointerVector<BoolView> local_skip_vector_list_;
...
FailureOrOwned<BoundExpression> BoundInputAttributeProjection(
  ...
  PROPAGATE_ON_FAILURE(result);
  return common::Success(new BoundInputProjectionExpression(result.release()));
}
...
FailureOrOwned<BoundExpression> BoundProjection(
    const BoundMultiSourceProjector* projector,
    BoundExpressionList* arguments) {
 return common::Success(new BoundProjectionExpression(projector, arguments));
}
