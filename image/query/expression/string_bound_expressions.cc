...
#include "image/query/expression/string_bound_expressions.h"
...
#include "image/query/expression/string_evaluators.h"  // IWYU pragma: keep
...
FailureOrOwned<BoundExpression> BoundToString(BoundExpression* arg,
  ...
  if (GetExpressionType(arg) == STRING) return common::Success(arg);
...
FailureOrOwned<BoundExpression> BoundConcat(BoundExpressionList* args,
  ...
  string name = base::StringPrintf("The %dth element on the concat list", i);
