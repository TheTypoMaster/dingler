...
#include "image/query/expression/stateful_bound_expressions.h"
...
#include "image/query/base/aggregation_operators.h"
...
struct Persister {
  ...
  FailureOrVoid Persist(bool initialized, void* state) { return common::Success(); }
...
struct Persister<true> {
  ...
  FailureOrVoid Persist(bool initialized, StringPiece* state) {
    if (!initialized) return common::Success();
...
class BoundRunningAggregationExpression : public BoundUnaryExpression {
 ...
 private:
  ...
  typename if_<TypeTraits<input_type>::is_variable_length,
