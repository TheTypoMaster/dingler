...
#ifndef IMAGE_BASE_EXCEPTION_EXCEPTION_H_
#define IMAGE_BASE_EXCEPTION_EXCEPTION_H_
...
class Exception {
 public:
  ...
  const std::string& message() const { return serial_.message(); }
  ...
  std::string PrintStackTrace() const {
    ...
    container::AppendStackTraceDump(serial_.stack_trace(), &out);
    return out;
  }
  ...
  const StackTrace& stack_trace() const {
    return serial_.stack_trace();
  }

  // Adds a new stack trace element to the stack trace.
  Exception* AddStackTraceElement(const base::StringPiece& function,
                                  const base::StringPiece& filename,
                                  int line,
                                  const base::StringPiece& context) {
   container::AddStackTraceElement(
...
#endif  // IMAGE_BASE_EXCEPTION_EXCEPTION_H_
