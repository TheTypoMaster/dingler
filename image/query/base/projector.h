...
#ifndef IMAGE_BASE_INFRASTRUCTURE_PROJECTOR_H_
#define IMAGE_BASE_INFRASTRUCTURE_PROJECTOR_H_
...
typedef std::multimap<SourceAttribute, int> ReverseProjectionMap;
...
class BoundMultiSourceProjector {
 public:
  ...
  std::pair<PositionIterator, PositionIterator> ProjectedAttributePositions(
...
class BoundSingleSourceProjector {
 public:
  ...
  string GetDebugString() const {
    ...
    for (int i = 0; i < result_schema().attribute_count(); ++i) {
      if (i > 0) result += ", ";
      base::StringAppendF(&result, "%d", source_attribute_position(i));
    }
    return result;
  }
...
std::pair<BoundMultiSourceProjector*, BoundSingleSourceProjector*> DecomposeNth(
    int source_index,
...
inline const SingleSourceProjector* ProjectRenameSingle(
    const StringPiece& alias,
    const SingleSourceProjector* source) {
  return ProjectRename(vector<string>(1, alias.as_string()), source);
}
...
class CompoundMultiSourceProjector : public MultiSourceProjector {
 public:
  ...
  CompoundMultiSourceProjector* add(int source_index,
    ...
    projectors_.push_back(std::make_pair(source_index, make_linked_ptr(projector)));
    return this;
  }
  virtual FailureOrOwned<const BoundMultiSourceProjector> Bind(
 ...
 private:
  ...
  vector<std::pair<int, linked_ptr<const SingleSourceProjector> > > projectors_;
...
#endif  // IMAGE_BASE_INFRASTRUCTURE_PROJECTOR_H_
