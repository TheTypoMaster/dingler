...
bool BoundMultiSourceProjector::AddAs(int source_index,
  ...
  reverse_projection_map_.insert(
      std::make_pair(projected_attribute, projection_map_.size()));
  projection_map_.push_back(projected_attribute);
  return true;
}

std::pair<PositionIterator, PositionIterator>
BoundMultiSourceProjector::ProjectedAttributePositions(
    int source_index, int attribute_position) const {
 const std::pair<ReverseProjectionMap::const_iterator,
...
FailureOrOwned<const BoundSingleSourceProjector> NamedAttributeProjector::Bind(
  ...
  if (source_position < 0) {
    THROW(new Exception(
        ...
        base::StringPrintf("No attribute '%s' in the schema:\n '%s'", name_.c_str(),
    ...
    return common::Success(projector.release());
  }
}
...
class RenamingProjector : public SingleSourceProjector {
 public:
  explicit RenamingProjector(const std::vector<std::string>& aliases,
  ...
  virtual FailureOrOwned<const BoundSingleSourceProjector> Bind(
    ...
    return common::Success(result_projector.release());
  }
...
string RenamingProjector::ToString(bool verbose) const {
  std::string result_description = base::StringPrintf("(%s) RENAME AS (",
                                           source_->ToString(verbose).c_str());
  for (std::vector<std::string>::const_iterator it = aliases_.begin();
      it < aliases_.end(); ++it) {
 ...
 public:
  ...
  virtual FailureOrOwned<const BoundSingleSourceProjector> Bind(
    ...
    if (source_position_ >= source_schema.attribute_count()) {
      THROW(new Exception(
          ERROR_ATTRIBUTE_COUNT_MISMATCH,
          base::StringPrintf("source schema has too few attributes (%d vs %d)",
  ...
  virtual string ToString(bool verbose) const {
   return base::StringPrintf("AttributeAt(%d)", source_position_);
  }
