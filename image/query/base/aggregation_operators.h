...
#ifndef IMAGE_BASE_INFRASTRUCTURE_AGGREGATION_OPERATORS_H_
#define IMAGE_BASE_INFRASTRUCTURE_AGGREGATION_OPERATORS_H_
...
struct AssignmentOperator<STRING, STRING, true> {
  ...
  inline bool operator()(const base::StringPiece& val,
    ...
    *result = base::StringPiece(static_cast<char*>(buffer->data()), val.length());
    return true;
  }
...
struct AggregationOperator<CONCAT, InputType, STRING, deep_copy> {
  ...
  inline bool operator()(const typename TypeTraits<InputType>::cpp_type& val,
   ...
   std::unique_ptr<Buffer>* buffer_ptr) {
    ...
    base::StringPiece val_string = as_string(val);
    ...
    *result = base::StringPiece(static_cast<char*>(buffer->data()),
                          concat_result_size);
    return true;
  }
...
#endif  // IMAGE_BASE_INFRASTRUCTURE_AGGREGATION_OPERATORS_H_
