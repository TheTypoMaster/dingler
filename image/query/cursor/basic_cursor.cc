...
#include "image/query/utils/demangle.h"  // For Cursor::GetDebugDescription().
...
string BasicCursor::DebugDescription() const {
  const std::string name = Demangle(typeid(*this).name());
  vector<std::string> split = strings::Split(name, "::");
  CHECK(!split.empty());
  return split.back();
}
