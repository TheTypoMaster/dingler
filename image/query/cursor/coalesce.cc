...
#include "image/query/cursor/coalesce.h"
...
#include "image/query/cursor/generate.h"
...
class CoalesceCursor : public BasicCursor {
 ...
 private:
  gtl::PointerVector<CursorIterator> inputs_;
...
FailureOrOwned<Cursor> BoundCoalesce(const vector<Cursor*>& children) {
  ...
  if (children.size() == 1) {
    ...
    return common::Success(children[0]);
  }
  ...
  return common::Success(new CoalesceCursor(children, bound_projector.release()));
}
