...
#include "image/query/cursor/sort.h"
...
#include "image/query/base/memory/memory.h"
...
#include "image/query/cursor/merge_union_all.h"
#include "image/query/cursor/ownership_taker.h"
#include "image/query/cursor/project.h"
#include "image/query/cursor/scan_view.h"
#include "image/query/cursor/basic_cursor.h"
#include "image/query/cursor/basic_operation.h"
#include "image/query/cursor/file_io.h"
#include "image/query/cursor/ordering.h"
#include "image/query/cursor/table.h"
#include "image/query/cursor/view_cursor.h"
#include "image/query/cursor/writer.h"
#include "image/query/expression/expression.h"
#include "image/query/expression/string_expressions.h"
#include "image/query/expression/projecting_bound_expressions.h"
#include "image/query/expression/projecting_expressions.h"
...
#include "image/query/utils/file.h"
#include "image/query/utils/file_util.h"
...
class BasicMerger : public Merger {
 public:
  BasicMerger(TupleSchema schema, StringPiece temporary_directory_prefix,
      ...
      : schema_(schema),
        temporary_directory_prefix_(temporary_directory_prefix.as_string()),
 ...
 public:
  ...
  FailureOrOwned<Cursor> GetResultCursorMergedWith(Cursor* sorted_cursor) {
    if (merger_->empty() && sorted_cursor != NULL) {
     return common::Success(sorted_cursor);
    } else {
      FailureOrOwned<Cursor> merged =
  ...
  FailureOrOwned<Cursor> SortView(const View& view) {
    ...
    PROPAGATE_ON_FAILURE(sorted);
    return common::Success(TakeOwnership(sorted.release(), permutation.release()));
  }
...
FailureOrOwned<Cursor> BoundExtendedSort(
  ...
  return common::Success(final_cursor.release());
}
