...
#include "image/query/cursor/file_io-internal.h"
...
class FileSink : public Sink {
 public:
  ...
  virtual FailureOr<rowcount_t> Write(const View& data) {
    ...
    return common::Success(data.row_count());
  }
...
FailureOr<ReadResult> Read(File* input_file,
  ...
  if (read == length) return common::Success(DATA);
  if (read == 0 && input_file->eof()) return common::Success(END_OF_FILE);
...
FailureOr<ReadResult> ReadRowCount(File* input_file, rowcount_t* datum) {
  ...
  PROPAGATE_ON_FAILURE(result);
  if (result.get() == END_OF_FILE) return common::Success(END_OF_FILE);
...
ResultView FileInputCursor::Next(const rowcount_t max_row_count) {
  ...
  if (rows_pending_in_block_ != 0) {
   rowcount_t rows_to_return = std::min(max_row_count, rows_pending_in_block_);
  ...
  rowcount_t rows_to_return = std::min(chunk_row_count, max_row_count);
