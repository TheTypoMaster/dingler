...
#include "image/query/cursor/splitter.h"
...
Cursor* BarrierSplitter::AddReader() {
 ...
 std::shared_ptr<BarrierSplitter> splitter = readers_.empty()
  ? std::shared_ptr<BarrierSplitter>(this)
      : readers_[0]->splitter();
...
Cursor* BufferedSplitter::AddReader() {
  ...
  // First reader takes ownership; subsequent readers share it.
  std::shared_ptr<BufferedSplitter> splitter = readers_.empty()
   ? std::shared_ptr<BufferedSplitter>(this)
      : readers_[0]->splitter();
...
FailureOrVoid BufferedSplitter::AllocateNewBlock() {
  ...
  if (storages_.empty()) {
   ...
   std::list<BlockWithMetadata>::iterator next_block = last_block_;
    ++next_block;
...
FailureOr<const View*> BufferedSplitter::NextView(size_t position) {
  ...
  if (next_row_position_[position] == rows_fetched_) {
    ...
    if (!input_.Next(max_row_count_, /* limit cursor input = */ false)) {
      ...
      return common::Success(static_cast<const View*>(NULL));
    }
  ...
  return common::Success(views_[position].get());
}
