...
#ifndef IMAGE_CURSOR_CORE_SPLITTER_H_
#define IMAGE_CURSOR_CORE_SPLITTER_H_
...
#include "image/query/cursor/row.h"
...
class BufferedSplitter : public SplitterInterface {
 ...
 private:
  ...
  gtl::PointerVector<Block> owned_storages_;
  ...
  std::list<BlockWithMetadata> storages_;

  // Points to the block that's last used to store data.
  std::list<BlockWithMetadata>::iterator last_block_;
...
class BufferedSplitReaderCursor : public Cursor {
 public:
  BufferedSplitReaderCursor(std::shared_ptr<BufferedSplitter> const splitter,
                            const int position)
      : splitter_(splitter),
        position_(position),
...
class BarrierSplitReaderCursor : public Cursor {
 public:
  BarrierSplitReaderCursor(std::shared_ptr<BarrierSplitter> const splitter,
                           const int position)
      : splitter_(splitter),
        position_(position),
        at_barrier_(true),
  ...
  std::shared_ptr<BarrierSplitter> const splitter() const { return splitter_; }

 private:
  std::shared_ptr<BarrierSplitter> const splitter_;  // shared by all Readers.
  size_t position_;  // Index of this reader in the splitter.
  bool at_barrier_;
  ViewIterator input_;
...
#endif  // IMAGE_CURSOR_CORE_SPLITTER_H_
