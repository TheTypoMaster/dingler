...
#include "image/query/cursor/hash_join.h"
...
#include "image/query/expression/vector_logic.h"
#include "image/query/utils/strings/join.h"
#include "image/query/utils/container_literal.h"
#include "image/query/utils/iterator_adaptors.h"
...
class HashIndexOnMaterializedCursor : public LookupIndex {
 ...
 private:
  ...
  typedef typename if_<key_uniqueness == UNIQUE,
...
class HashIndexMaterializer : public LookupIndexBuilder {
 public:
  static FailureOrOwned<LookupIndexBuilder> Create(
    ...
    return common::Success(materializer.release());
  }
  ...
  FailureOrOwned<LookupIndex> Build() {
    ...
    if (materialized.get()) {
      ...
      return common::Success(index_.release());
    } else {
     return common::Success(static_cast<LookupIndex*>(NULL));
    }
  }
...
FailureOrOwned<Cursor> HashJoinOperation::CreateCursor() const {
  ...
  FailureOrOwned<const BoundMultiSourceProjector> bound_result_projector =
      result_projector_->Bind(gtl::Container(
  ...
  return common::Success(cursor.release());
}
...
int ProjectedAttributePosition(
 ...
 std::pair<PositionIterator, PositionIterator> positions =
...
void HashJoinCursor::SetUpFinalResultProjector(
  ...
  final_result_projector_.reset(new BoundMultiSourceProjector(
      gtl::Container(&lhs_result_.schema(), &rhs_builder_->schema()).
...
MaterializeInputAndBuildIndex(Cursor* input) {
  ...
  PROPAGATE_ON_FAILURE(result);
  return common::Success(result.is_eos());
}
...
class HashIndexOnMaterializedCursor<key_uniqueness>::ResultCursor
 ...
 private:
  ...
  inline void ProcessNextResult() {
    typename if_<key_uniqueness == UNIQUE, Unique, NonUnique>::type
        dispatch;
    dispatch(this);
  }
...
::MultiLookup(const View* query) const {
  ...
  switch (join_type_) {
    case INNER:
     return common::Success(new ResultCursor<INNER>(
          index_, *query, &result_cursor_block_,
          result_cursor_query_ids_.get()));
    case LEFT_OUTER:
     return common::Success(new ResultCursor<LEFT_OUTER>(
          index_, *query, &result_cursor_block_,
          result_cursor_query_ids_.get()));
    default:
