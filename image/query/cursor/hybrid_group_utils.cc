...
using container::gtl::PointerVector;
...
FailureOrOwned<Cursor> HybridGroupTransformCursor::Create(
  ...
  return common::Success(new HybridGroupTransformCursor(
      transformed_schema,
      nulls_block_schema,
...
FailureOrOwned<Cursor> BoundHybridGroupTransform(
    const SingleSourceProjector* group_by_columns,
    const gtl::PointerVector<const SingleSourceProjector>&
