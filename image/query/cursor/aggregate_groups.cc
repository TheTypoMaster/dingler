...
#include "image/query/cursor/hybrid_group_utils.h"
...
#include "image/query/cursor/row_hash_set.h"
#include "image/query/cursor/table.h"
#include "image/query/utils/supersonic.pb.h"
#include "image/query/utils/strings/join.h"
#include "image/query/utils/container_literal.h"
#include "image/query/utils/map_util.h"
...
class GroupKeySet {
 public:
  ...
  static FailureOrOwned<GroupKeySet> Create(
    ...
    return common::Success(group_key_set.release());
  }
...
class GroupAggregateCursor : public BasicCursor {
 public:
  ...
  static FailureOrOwned<GroupAggregateCursor> Create(
      ...
      const int64 max_unique_keys_in_result,
      Cursor* child) {
    std::unique_ptr<BufferAllocator> allocator_owned(allocator);
...
class HybridGroupSetup {
 public:
  ...
  static FailureOrOwned<const HybridGroupSetup> Create(
    ...
    PROPAGATE_ON_FAILURE(setup->Init(group_by_columns,
                                     ...
                                     input_schema));
    return common::Success(setup.release());
  }
  ...
  FailureOr<TupleSchema> ComputeResultSchema(
    ...
    return common::Success(result_schema);
  }
 ...
 private:
      ...
      if (elem.is_distinct()) {
        ...
        if (elem.input() == "") {
          THROW(new Exception(
              ...
              base::StringPrintf("Incorrect aggregation specification. Distinct "
  ...
  static FailureOrOwned<const SingleSourceProjector>
    ...
    return common::Success(ProjectNamedAttributes(result_names));
  }
 ...
 private:
  ...
  gtl::PointerVector<const SingleSourceProjector>
      column_group_projectors_;
...
class HybridGroupFinalAggregationCursor : public BasicCursor {
 public:
  ...
  HybridGroupFinalAggregationCursor(
      ...
      base::StringPiece temporary_directory_prefix,
      BufferAllocator* allocator,
...
FailureOrOwned<Cursor> BoundGroupAggregateWithLimit(
  ...
  PROPAGATE_ON_FAILURE(result);
  return common::Success(result.release());
}
...
FailureOrOwned<Cursor> BoundHybridGroupAggregate(
  ...
  return common::Success(new HybridGroupFinalAggregationCursor(
      result_schema.get(),
