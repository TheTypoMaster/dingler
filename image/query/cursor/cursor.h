...
#ifndef IMAGE_CURSOR_BASE_CURSOR_H_
#define IMAGE_CURSOR_BASE_CURSOR_H_

#include <string>

#include "base/logging.h"
#include "image/query/utils/macros.h"
#include "image/query/utils/coowned_pointer.h"
...
class ResultView {
 public:
  ...
  ResultView(const common::FailurePropagator<Exception>& failure)  // NOLINT
   : result_(common::Failure(failure.exception)),
 ...
 protected:
  ResultView(const View* view, ReturnCode status)
   : result_(container::common::Success(view)),
        status_(status) {}

  explicit ResultView(Exception* exception)
   : result_(container::common::Failure(exception)),
...
#endif  // IMAGE_CURSOR_BASE_CURSOR_H_
