...
#include "kernel/kernel_process_finder_win.h"
#include "common/constants.h"
#include "common/paths.h"
#include "common/switches.h"
#include "kernel/kernel_watcher/exit_funnel_win.h"
...
namespace {
 ...
 BOOL CALLBACK ShellWindowEnumeration(HWND window, LPARAM param) {
 ...
 bool DisplayShouldKillMessageBox() {
  /*return kernel::ShowMessageBox(
   NULL, l10n_util::GetStringUTF16(IDS_PRODUCT_NAME),
   l10n_util::GetStringUTF16(IDS_BROWSER_HUNGBROWSER_MESSAGE),
   kernel::MESSAGE_BOX_TYPE_QUESTION) !=
   kernel::MESSAGE_BOX_RESULT_NO;*/
  return false;
 }
}  // namespace
...
bool ProcessSingleton::EscapeVirtualization(
 ...
 //  hwnd = kernel::FindRunningWindow(user_data_dir);
 //  if (hwnd) {
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcess() {
 ...
 switch (kernel::AttemptToNotifyRunningChrome(remote_window_, false)) {
 case kernel::NOTIFY_SUCCESS:
  return PROCESS_NOTIFIED;
 case kernel::NOTIFY_FAILED:
  remote_window_ = NULL;
  return PROCESS_NONE;
 case kernel::NOTIFY_WINDOW_HUNG:
  // Record a hung rendezvous event in this process' exit funnel.
  /*kernel::ExitFunnel::RecordSingleEvent(
   kernel::kShellExitCodesRegistryPath, L"RendezvousToHungShell");*/
  break;
 }
 DWORD process_id = 0;
 ...
 //kernel::ExitFunnel funnel;
 /*if (funnel.Init(kernel::kShellExitCodesRegistryPath, process.Handle()))
  funnel.RecordEvent(L"HungShellTerminated");*/
 // Time to take action. Kill the browser process.
 process.Terminate(common::RESULT_CODE_HUNG, true);
 remote_window_ = NULL;
 return PROCESS_NONE;
}
...
ProcessSingleton::NotifyOtherProcessOrCreate() {
 ...
 /*else {
  g_shell_context->platform_part()->PlatformSpecificCommandLineProcessing(
...
bool ProcessSingleton::Create() {
 ...
 remote_window_ = kernel::FindRunningWindow(user_data_dir_);
