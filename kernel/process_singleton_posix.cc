...
#include "modules/shell/process_singleton.h"
...
#include "modules/shell/shell_thread.h"
...
bool IsChromeProcess(pid_t pid) {
  ...
  return (!other_chrome_path.empty() &&
          ...
          base::FilePath(constants::kShellProcessExecutableName));
}
...
bool ConnectSocket(ScopedSocket* socket,
  ...
  if (base::ReadSymbolicLink(socket_path, &socket_target)) {
    ...
    base::FilePath remote_cookie = socket_target.DirName().
                             Append(constants::kSingletonCookieFilename);
...
class ProcessSingleton::LinuxWatcher
   ...
   public:
    SocketReader(ProcessSingleton::LinuxWatcher* parent,
        ...
        : parent_(parent),
          ...
          fd_(fd),
          bytes_read_(0) {
      DCHECK(ShellThread::CurrentlyOn(ShellThread::IO));
      // Wait for reads.
 ...
 private:
  friend struct ShellThread::DeleteOnThread<ShellThread::IO>;
...
ProcessSingleton::ProcessSingleton(
  ...
  socket_path_ = user_data_dir.Append(constants::kSingletonSocketFilename);
  lock_path_ = user_data_dir.Append(constants::kSingletonLockFilename);
  cookie_path_ = user_data_dir.Append(constants::kSingletonCookieFilename);
...
bool ProcessSingleton::Create() {
  ...
  base::FilePath socket_target_path =
      socket_dir_.path().Append(constants::kSingletonSocketFilename);
  base::FilePath cookie(GenerateCookie());
  base::FilePath remote_cookie_path =
      socket_dir_.path().Append(constants::kSingletonCookieFilename);
  ...
  DCHECK(ShellThread::IsMessageLoopValid(ShellThread::IO));
  ShellThread::PostTask(
      ShellThread::IO,
      FROM_HERE,
