...
void FileDescriptorInfoImpl::Transfer(int id, base::win::ScopedHandle fd) {
  AddToMapping(id, fd.Get());//fd.get());
  //owned_descriptors_.push_back(new base::ScopedFD(fd.Pass()));
  owned_descriptors_.push_back(new  base::win::ScopedHandle(fd.Pass()));
}
